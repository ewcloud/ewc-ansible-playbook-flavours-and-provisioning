...
### Manually update DNS nameserver(s)
...
#### 1. Note down the private IP address of the VM newly configured as IPA server
...
#### 2. Ensure the IPA server's IP address is part of the list of subnet DSN nameservers
...
```bash
ewc hub deploy ipa-server-flavour
```
